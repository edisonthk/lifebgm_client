...
$(function () {
    ...
    function onDisconnectble(event) {
        Log.w(TAG, "disconnectble status:" + event.status);
        if (event.status == 0) {
    ...
    function onRegisterforNotification(event) {
        Log.d(TAG, "onRegisterforNotification registered:" + event.registered + " char_id_uuid:" + event.char_id_uuid);
    ...
    $('#char_write').on('click', function() {
     ...
     $('#char_notify').on('click', function() {
        Log.d(TAG, "click notify characteristic:" + select_char_id.uuid);
        bleManager.registerForNotification(client_if, bd_addr, select_srvc_id, select_char_id);
        /*
        registerForNotification(client_if, bd_addr, select_srvc_id, select_char_id);
        deregisterForNotification(client_if, bd_addr, select_srvc_id, select_char_id);
        */
    });
...
/*
        for(var key in select_srvc_id){
            console.log(key+":"+select_srvc_id[key]);
        }
*/